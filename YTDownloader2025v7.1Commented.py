...
class YoutubeDownloader:
    ...
    def download_video(self, url, download_dir, selected_format, filename_template=None):
        ...
        try:
            ...
            if selected_format == 'mp3':  # If audio-only download
            ...
            elif selected_format.startswith('best'):  # If specific best format
                ydl_opts.update({  # Update options for specific format
                    'format': selected_format + '+bestaudio/best',  # Add best audio
                    'merge_output_format': 'mp4',  # Merge to MP4
                    'prefer_ffmpeg': True,  # Prefer FFmpeg
                })
                ...
                ydl_opts.update({  # Update options for container format
                    'format': 'bestvideo+bestaudio/best',  # Get best video and audio
                    'merge_output_format': selected_format,  # Use specified format
                    'prefer_ffmpeg': True,  # Prefer FFmpeg
                })
            
            # Try quality-based selection
            try:
                height = int(selected_format.rstrip('p'))  # Extract height from format
                ydl_opts.update({  # Update options for specific quality
                    'format': f'bestvideo[height<={height}]+bestaudio/best[height<={height}]',  # Limit height
                    'merge_output_format': 'mp4',  # Merge to MP4
                    'prefer_ffmpeg': True,  # Prefer FFmpeg
                })
            except ValueError:  # If format is not a resolution
            ...
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:  # Create yt-dlp instance
                ...
                info = self.current_info or ydl.extract_info(url, download=False)  # Get video info
                ...
                if self.cancelled:  # If cancelled
                ...
                self._cleanup_temp_files(download_dir)  # Remove temp files
            ...
            self._cleanup_temp_files(download_dir)  # Clean up files
    ...
    def _cleanup_temp_files(directory):
        ...
        try:
            # Define temporary file extensions
            temp_extensions = [
                '.part', '.temp', '.f140.mp4', '.f137.mp4', 
                '.f401.mp4', '.m4a', '.webm', '.ytdl'  # Common temp extensions
            ]
            
            # Process each file in directory
            for file in os.listdir(directory):  # List directory contents
                # Check if file is temporary
                is_temp = any(file.endswith(ext) for ext in temp_extensions)  # Check extensions
                
                # Remove temporary files
                if is_temp:  # If file is temporary
                    try:
                        os.remove(os.path.join(directory, file))  # Remove file
                        logging.debug(f"Cleaned up: {file}")  # Log removal
                    except (IOError, OSError) as e:  # Handle removal errors
                        logging.warning(f"Could not remove temporary file {file}: {e}")  # Log warning
        
        except Exception as e:  # Handle other errors
            logging.warning(f"Cleanup error: {e}")  # Log cleanup error
...
class DownloadManagerApp:
    ...
    def __init__(self, root):
        ...
        if os.name == 'nt':  # If on Windows
            self.root.tk_setPalette(background='#f0f0f0')  # Set palette
        
        # Initialize application variables
        self.download_directory = DEFAULT_DOWNLOAD_DIR  # Set default directory
        ...
        self.available_formats = []  # Initialize formats list
        self.video_info = None  # Initialize video info
        self.thumbnail_image = None  # Initialize thumbnail
        self.custom_filename = None  # Initialize filename
        self.active_download_thread = None  # Initialize download thread
        self.thumbnail_load_thread = None  # Initialize thumbnail thread
        self.download_in_progress = False  # Initialize download state
        
        # Configure application styles
        self._configure_styles()  # Set up styles
        
        # Create main frame
        self.main_frame = ttk.Frame(root, padding="20")  # Create frame with padding
        self.main_frame.pack(fill=tk.BOTH, expand=True)  # Pack frame
        ...
        if not is_ffmpeg_installed():  # If FFmpeg not found
    ...
    def _create_directory_section(self):
        ...
        dir_frame = ttk.LabelFrame(self.main_frame, text="Download Location", padding="5")  # Create frame
        dir_frame.pack(fill=tk.X, pady=(0, 10))  # Pack frame with padding
    ...
    def _create_url_section(self):
        ...
        url_frame = ttk.LabelFrame(self.main_frame, text="Video URL", padding="5")  # Create frame
        url_frame.pack(fill=tk.X, pady=(0, 10))  # Pack frame with padding
    ...
    def _create_thumbnail_section(self):
        ...
        thumbnail_frame = ttk.LabelFrame(self.main_frame, text="Video Preview", padding="5")  # Create frame
        thumbnail_frame.pack(fill=tk.X, pady=(0, 10))  # Pack frame with padding
    ...
    def _create_format_section(self):
        ...
        format_frame = ttk.LabelFrame(self.main_frame, text="Format Type", padding="5")  # Create frame
        format_frame.pack(fill=tk.X, pady=(0, 10))  # Pack frame with padding
    ...
    def _create_quality_section(self):
        ...
        quality_frame = ttk.LabelFrame(self.main_frame, text="Quality Options", padding="5")  # Create frame
        quality_frame.pack(fill=tk.BOTH, expand=True, pady=(0, 10))  # Pack frame with padding
    ...
    def _create_progress_section(self):
        ...
        progress_frame = ttk.LabelFrame(self.main_frame, text="Download Progress", padding="5")  # Create frame
        progress_frame.pack(fill=tk.X, pady=(0, 10))  # Pack frame with padding
    ...
    def _create_control_section(self):
        ...
        control_frame = ttk.Frame(self.main_frame)  # Create frame
        control_frame.pack(fill=tk.X, padx=5, pady=5)  # Pack frame
