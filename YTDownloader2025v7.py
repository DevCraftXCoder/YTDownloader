    ...
    def _download_thread_func(self, url, download_dir, selected_format, ydl_opts):
        ...
        try:
            ...
            if selected_format == 'mp3':
                ...
                ydl_opts.update({
                    'format': f'bestvideo[height<={selected_format.rstrip("p")}]+bestaudio/best[height<={selected_format.rstrip("p")}]',
                    'merge_output_format': 'mp4',
                    'prefer_ffmpeg': True,
                })
    ...
    def toggle_speed_boost(self):
        ...
        if self.speed_boost_active:
            self.speed_boost_button.configure(
                text="⚡ Speed Boost ON",
                style="SpeedBoostActive.TButton"
            )
            messagebox.showinfo(
                "Speed Boost Activated",
                "Speed boost mode enabled!\n"
            ...
            self.speed_boost_button.configure(
                text="🚀 Speed Boost",
                style="SpeedBoost.TButton"
            )
            messagebox.showinfo(
                "Speed Boost Deactivated",
                "Speed boost mode disabled.\n"
